...
import 'package:vigil_erp/screens/definitions/stocks/scr_StocksItem.dart';
...
class scr_StocksView extends StatefulWidget {
  scr_StocksView({super.key});

  @override
  State<scr_StocksView> createState() => _scr_StocksViewState();
}

class _scr_StocksViewState extends State<scr_StocksView> {
            ...
            children: [
              ...
              BlocBuilder< stock_bloc , definition_state>(
                builder: (context, state) {
                  if (state is getListStock_StateDataChanged)
                    return ctr_TextHeaderPage(
                      text: state.filterdLst_Stock.length.toString(),
                      color: Colors.grey[300],
                      borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
            ...
            child: Row(
              children: [
                Expanded(
                  ...
                  OnChanged: (value) {
                    if (value != null) {
                      stock_bloc.instance.filterAny_Stock(filterData: value.trim());
                    }
                      return null;
                ...
                IconButton(
                  onPressed: () {
                    ...
                    stock_bloc.instance.resetFilter_Stock();
                  },
  ...
  Widget buildListView(BuildContext context) {
    return  BlocBuilder<stock_bloc, definition_state>(
      builder: (context, state) {
        if (state is stock_StateInitial) {
          const LinearProgressIndicator(minHeight: 6);
          return const Center(child: CircularProgressIndicator());
        } else if (state is getListStock_StateDataChanged) {
                  ...
                  child: SizedBox(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        return buildListViewItem(state.filterdLst_Stock[index], context);
                      },
                      separatorBuilder: (context, index) => const SizedBox(height: 1),
                      itemCount: state.filterdLst_Stock.length,
                    ),
                  ),
                ),
              ],
  ...
  buildListViewItem(Def_Stocks item, context) {
    return Column(
      children: [
        Divider(
          color: Colors.grey,
  ...
  Future loadDataFromDB() async {
    stock_bloc.instance.add(getListStock_Event([BLLCondions(enTable_Def_Stocks.IsActive.name, en_CondionsWhere.isEqualTo, true)]));
    if (controllerfilter.text.trim().isNotEmpty) {
      stock_bloc .instance.add(filterAnyStock_Event(filterData: controllerfilter.text.trim()));
    }
  }
  ...
  void newItem() async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_StocksItem(null, scaffold, en_FormMode.NewMode),
      ),
    );
  ...
  void editItem(Def_Stocks item) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_StocksItem(item, scaffold, en_FormMode.EditMode),
      ),
    );
  ...
  void deleteItem(Def_Stocks item) {
          ...
          Row(
            children: [
              ElevatedButton.icon(
                ...
                onPressed: () {
                  bllDef_Stocks.fire_DeleteItem(item.ID.toString());
                  loadDataFromDB();
                  Navigator.of(context).pop();
                },
              ),
  ...
  void shareItem(Def_Stocks item) async {
