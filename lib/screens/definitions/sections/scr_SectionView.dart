...
import 'package:vigil_erp/screens/definitions/Sections/scr_SectionItem.dart';
...
class scr_SectionView extends StatefulWidget {
  scr_SectionView({super.key});

  @override
  State<scr_SectionView> createState() => _scr_SectionViewState();
}

class _scr_SectionViewState extends State<scr_SectionView> {
      ...
      child: Column(
        children: [
          Center(
            ...
            children: [
              ctr_TextHeaderPage(
                text: 'الإدارات والأقسام',
                style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 25),
                color: Colors.grey[300],
                borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
              ),

              BlocBuilder< sections_bloc , definition_state>(
                builder: (context, state) {
                  if (state is getListSections_StateDataChanged)
                    return ctr_TextHeaderPage(
                      text: state.filterdLst_Sections.length.toString(),
                      color: Colors.grey[300],
                      borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
            ...
            child: Row(
              children: [
                Expanded(
                  ...
                  OnChanged: (value) {
                    if (value != null) {
                      sections_bloc.instance.filterAny_Section(filterData: value.trim());
                    }
                      return null;
                ...
                IconButton(
                  onPressed: () {
                    ...
                    sections_bloc.instance.resetFilter_Section();
                  },
  ...
  Widget buildListView(BuildContext context) {
    return  BlocBuilder<sections_bloc, definition_state>(
      builder: (context, state) {
        if (state is sections_StateInitial) {
          const LinearProgressIndicator(minHeight: 6);
          return const Center(child: CircularProgressIndicator());
        } else if (state is getListSections_StateDataChanged) {
                  ...
                  child: SizedBox(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        return buildListViewItem(state.filterdLst_Sections[index], context);
                      },
                      separatorBuilder: (context, index) => const SizedBox(height: 1),
                      itemCount: state.filterdLst_Sections.length,
                    ),
                  ),
                ),
              ],
  ...
  buildListViewItem(Def_Sections item, context) {
    return Column(
      children: [
        Divider(
          color: Colors.grey,
  ...
  Future loadDataFromDB() async {
    sections_bloc.instance.add(getListSections_Event([BLLCondions(enTable_Def_Sections.IsActive.name, en_CondionsWhere.isEqualTo, true)]));
    if (controllerfilter.text.trim().isNotEmpty) {
      sections_bloc .instance.add(filterAnySections_Event(filterData: controllerfilter.text.trim()));
    }
  }
  ...
  void newItem() async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_SectionItem(null, scaffold, en_FormMode.NewMode),
      ),
    );
  ...
  void editItem(Def_Sections item) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_SectionItem(item, scaffold, en_FormMode.EditMode),
      ),
    );
  ...
  void deleteItem(Def_Sections item) {
          ...
          Row(
            children: [
              ElevatedButton.icon(
                ...
                onPressed: () {
                  bllDef_Sections.fire_DeleteItem(item.ID.toString());
                  loadDataFromDB();
                  Navigator.of(context).pop();
                },
              ),
  ...
  void shareItem(Def_Sections item) async {
