...
import 'package:vigil_erp/screens/financial/4-Transfere/scr_TransfereItem.dart';
...
class scr_TransfereView extends StatefulWidget {
  scr_TransfereView({super.key});

  @override
  State<scr_TransfereView> createState() => _scr_TransfereViewState();
}

class _scr_TransfereViewState extends State<scr_TransfereView> {
        ...
        child: Column(
          children: [
            Center(
              ...
              children: [
                ctr_TextHeaderPage(
                  text: 'عرض تحويلات النقدية',
                  style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 25),
                  color: Colors.grey[300],
                  borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
                ),
                BlocBuilder<transfereMony_bloc, financial_state>(
                  builder: (context, state) {
                    if (state is getListTransfere_StateDataChanged)
                      return ctr_TextHeaderPage(
                        text: state.filterdLst_Transfere.length.toString(),
                        color: Colors.grey[300],
                        borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
              ...
              child: Row(
                children: [
                  Expanded(
                    ...
                    OnChanged: (value) {
                      if (value != null) {
                        transfereMony_bloc.instance.add(filterAnyTransfereMony_Event(filterData: value.trim()));
                      }
                      return null;
                    },
                  )),
                  IconButton(
                    onPressed: () {
                      ...
                      transfereMony_bloc.instance.add(resetFilterTransfereMony_Event());
                    },
  ...
  Widget buildListView(BuildContext context) {
    return BlocBuilder<transfereMony_bloc, financial_state>(
      builder: (context, state) {
        if (state is financialnitial) {
          return const Center(child: CircularProgressIndicator());
        } else if (state is getListTransfere_StateDataChanged) {
          return SingleChildScrollView(
                    ...
                    child: SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          return buildListViewItem(state.filterdLst_Transfere[index], context);
                        },
                        separatorBuilder: (context, index) => const SizedBox(height: 1),
                        itemCount: state.filterdLst_Transfere.length,
                      ),
                    ),
                  ),
                      ...
                      children: [
                        ...
                        BlocBuilder<transfereMony_bloc, financial_state>(
                          builder: (context, state) {
                            if (state is getListTransfere_StateDataChanged) {
                              contTotalValue.text =
                                  state.filterdLst_Transfere.fold(0.0, (previousValue, element) => previousValue + element.Value!).toStringAsFixed(2);
                              return SizedBox(
                                width: 200,
                                child: ctr_TextFormField(
  ...
  buildListViewItem(Fina_Transfere item, context) {
    return Column(
      children: [
        Divider(
          color: Colors.grey,
  ...
  Future loadDataFromDB() async {
    ...
    transfereMony_bloc.instance.add(getListTransfereMony_Event());
    if (controllerfilter.text.trim().isNotEmpty) {
      transfereMony_bloc.instance.add(filterAnyTransfereMony_Event(filterData: controllerfilter.text.trim()));
    }
  }
  ...
  void newItem() async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_TransfereItem(null, scaffold, en_FormMode.NewMode),
      ),
    );
  ...
  void editItem(Fina_Transfere item) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_TransfereItem(item, scaffold, en_FormMode.EditMode),
      ),
    );
    ...
    return Row(
      children: [
        IconButton(
          onPressed: () {
            sharedControls
                .showFormFilterByDates(context, en_TablesName.Fina_Transfere, branchFromID, contDateFrom, contDateTo, isGetAllDates)
                .then((retValues) {
              if (retValues != null) {
        ...
        IconButton(
          onPressed: () {
            tablesCondions
                ...
                .then((cond) => transfereMony_bloc.instance.add(getListTransfereMony_Event(conditions: cond)));
          },
          icon: Icon(
  ...
  void deleteItem(Fina_Transfere item) {
          ...
          Row(
            children: [
              ElevatedButton.icon(
                ...
                onPressed: () {
                  bllFina_Transfere.fire_DeleteItem(item.ID.toString());
                  loadDataFromDB();
                  Navigator.of(context).pop();
                },
              ),
  ...
  void shareItem(Fina_Transfere item) async {
