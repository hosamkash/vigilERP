...
import 'package:vigil_erp/bll/bllFirebase/bllFina_Supplay.dart';
...
class scr_SupplayItem extends StatefulWidget {
  scr_SupplayItem(this.itemSupplay, this.scaffold, this.frmMode, {super.key});
  ...
  @override
  State<scr_SupplayItem> createState() => _scr_SupplayItemState();
}

class _scr_SupplayItemState extends State<scr_SupplayItem> {
  int selectedID = -1;
  int? branchID;
  ...
  void newMode() async {
    bllFina_Supplay.getMax_firestore(enTable_Fina_Supplay.Code).then((val) {
  ...
  void editMode() async {
    if (widget.itemSupplay != null) {
      selectedID = widget.itemSupplay!.ID!;
      branchID = widget.itemSupplay!.IDBranch;
      _branchID = ValueNotifier(branchID);
     // _branchID!.value = branchID  ;
      treasure_bloc.instance.add(getListTreasureAsDataSource_Event(branchID: branchID));

      treasurID = widget.itemSupplay!.IDTreasur;
      contCode.text = widget.itemSupplay!.Code.toString();
      contDate.text = widget.itemSupplay!.Date!;
      contTime.text = widget.itemSupplay!.Time!;

      dealingTypeID = widget.itemSupplay!.DealingTypeID;
      setDealingTypeByID();
      dealingID = widget.itemSupplay!.DealingID;
      setDealingByID();

      contDealingBalanceBefor.text = widget.itemSupplay!.BalanceBefor.toString();
      contValue.text = widget.itemSupplay!.Value.toString();
      financialClusesID = widget.itemSupplay!.IDFinancialCluses;
      contNote.text = widget.itemSupplay!.Note!;
      chkIsBindeDocument = widget.itemSupplay!.IsBindeDocument!;
      chkIsAccountedByRepresent = widget.itemSupplay!.IsAccountedByRepresent!;
      chkIsClosedTreasure = widget.itemSupplay!.IsClosedTreasure!;
      chkIsClosedEdit = widget.itemSupplay!.IsClosedEdit!;
    }
  }
  ...
  bool checkIsSavedClosed() {
    if (widget.frmMode == en_FormMode.EditMode && (widget.itemSupplay!.IsClosedEdit!))
      return true;
    else
      return false;
  }
  ...
  void saveData() async {
    if (frmKey.currentState != null && frmKey.currentState!.validate()) {
      if (widget.frmMode == en_FormMode.NewMode) {
        selectedID = await bllFina_Supplay.getMaxID_firestore();
      } else if (widget.frmMode == en_FormMode.EditMode) {
        selectedID = widget.itemSupplay!.ID!;
      }
      widget.itemSupplay = Fina_Supplay();
      widget.itemSupplay!.ID = selectedID;
      widget.itemSupplay!.IDBranch = branchID;
      widget.itemSupplay!.Code = int.parse(contCode.text);
      widget.itemSupplay!.Date = contDate.text;
      widget.itemSupplay!.Time = contTime.text;
      widget.itemSupplay!.IDTreasur = treasurID;
      widget.itemSupplay!.DealingTypeID = dealingTypeID;
      widget.itemSupplay!.DealingID = dealingID;
      widget.itemSupplay!.BalanceBefor = double.tryParse(contDealingBalanceBefor.text);
      widget.itemSupplay!.Value = double.tryParse(contValue.text);
      widget.itemSupplay!.IDFinancialCluses = financialClusesID;
      widget.itemSupplay!.Note = contNote.text;
      widget.itemSupplay!.IsBindeDocument = chkIsBindeDocument;
      widget.itemSupplay!.IsAccountedByRepresent = chkIsAccountedByRepresent;
      widget.itemSupplay!.IsClosedTreasure = chkIsClosedTreasure;
      widget.itemSupplay!.IsClosedEdit = chkIsClosedEdit;

      await bllFina_Supplay.fire_SetItem(selectedID.toString(), widget.itemSupplay!);
