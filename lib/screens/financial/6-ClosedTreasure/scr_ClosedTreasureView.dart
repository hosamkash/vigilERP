...
import 'package:vigil_erp/screens/financial/6-ClosedTreasure/scr_ClosedTreasureItem.dart';
import 'package:vigil_erp/shared/enumerators.dart';
import 'package:vigil_erp/shared/shared_controls.dart';
import '../../../bll/bllFirebase/bllFina_ClosedTreasure.dart';
import '../../../bll/classModel/Fina_ClosedTreasure.dart';

class scr_ClosedTreasureView extends StatefulWidget {
  scr_ClosedTreasureView({super.key});

  @override
  State<scr_ClosedTreasureView> createState() => _scr_ClosedTreasureViewState();
} 

class _scr_ClosedTreasureViewState extends State<scr_ClosedTreasureView> {
        ...
        child: Column(
          children: [
            Center(
              ...
              children: [
                ctr_TextHeaderPage(
                  text: 'عرض تقفيل الخزينة',
                  style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 25),
                  color: Colors.grey[300],
                  borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
                ),
                BlocBuilder<closedTreasure_bloc, financial_state>(
                  builder: (context, state) {
                    if (state is getListClosedTreasure_StateDataChanged)
                      return ctr_TextHeaderPage(
                        text: state.filterdLst_ClosedTreasure.length.toString(),
                        color: Colors.grey[300],
                        borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
              ...
              child: Row(
                children: [
                  Expanded(
                    ...
                    OnChanged: (value) {
                      if (value != null) {
                        closedTreasure_bloc.instance.add(filterAnyClosedTreasure_Event(filterData: value.trim()));
                      }
                      return null;
                    },
                  )),
                  IconButton(
                    onPressed: () {
                      ...
                      closedTreasure_bloc.instance.add(resetFilterClosedTreasure_Event());
                    },
  ...
  Widget buildListView(BuildContext context) {
    return BlocBuilder<closedTreasure_bloc, financial_state>(
      builder: (context, state) {
        if (state is financialnitial) {
          return const Center(child: CircularProgressIndicator());
        } else if (state is getListClosedTreasure_StateDataChanged) {
          return SingleChildScrollView(
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        SizedBox(
                            width: 100,
                            child: Text('من فرع', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                        SizedBox(
                            width: 100,
                            child: Text('من خزينة', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                        SizedBox(
                            width: 100,
                            child: Text('الموظف', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                        SizedBox(
                            width: 100,
                            child: Text('إلى فرع', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                        SizedBox(
                            width: 100,
                            child: Text('إلى خزينة', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                        SizedBox(
                            width: 70,
                            child: Text('المبلغ', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                        SizedBox(
                            width: 200,
                    ...
                    child: SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          return buildListViewItem(state.filterdLst_ClosedTreasure[index], context);
                        },
                        separatorBuilder: (context, index) => const SizedBox(height: 1),
                        itemCount: state.filterdLst_ClosedTreasure.length,
                      ),
                    ),
                  ),
                      ...
                      children: [
                        ...
                        BlocBuilder<closedTreasure_bloc, financial_state>(
                          builder: (context, state) {
                            if (state is getListClosedTreasure_StateDataChanged) {
                              contTotalValue.text =
                                  state.filterdLst_ClosedTreasure.fold(0.0, (previousValue, element) => previousValue + element.Value!).toStringAsFixed(2);
                              return SizedBox(
                                width: 200,
                                child: ctr_TextFormField(
  ...
  buildListViewItem(Fina_ClosedTreasure item, context) {
    return Column(
      children: [
        Divider(
          color: Colors.grey,
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                SizedBox(
                  width: 100,
                  child: Text(company_bloc.instance.getNameByID(item.IDBranchFrom),
                      textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                ),
                SizedBox(
                  width: 100,
                  child: Text(treasure_bloc.instance.getNameByID(item.IDTreasurFrom),
                      textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                ),
                SizedBox(
                  width: 100,
                ...
                SizedBox(
                  width: 100,
                  child: Text(company_bloc.instance.getNameByID(item.IDBranchTo),
                      textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                ),
                SizedBox(
                  width: 100,
                  child: Text(treasure_bloc.instance.getNameByID(item.IDTreasurTo),
                      textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                ),
                SizedBox(
                  width: 70,
  ...
  Future loadDataFromDB() async {
    ...
    closedTreasure_bloc.instance.add(getListClosedTreasure_Event());
    if (controllerfilter.text.trim().isNotEmpty) {
      closedTreasure_bloc.instance.add(filterAnyClosedTreasure_Event(filterData: controllerfilter.text.trim()));
    }
  }



  void editItem(Fina_ClosedTreasure item) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_ClosedTreasureItem(item, scaffold, en_FormMode.EditMode),
      ),
    );
    ...
    return Row(
      children: [
        IconButton(
          onPressed: () {
            sharedControls
                .showFormFilterByDates(context, en_TablesName.Fina_ClosedTreasure, branchFromID, contDateFrom, contDateTo, isGetAllDates)
                .then((retValues) {
              if (retValues != null) {
                isGetAllDates = retValues[0] as bool;
                contDateFrom.text = retValues[1] as String;
                contDateTo.text = retValues[2] as String;
                branchFromID = retValues[3] as int?;
              }
            });
          },
          icon: Icon(
        ...
        IconButton(
          onPressed: () {
            tablesCondions
                .createCondionsByDates(en_TablesName.Invoices_Sales, branchFromID, contDateFrom, contDateTo, isGetAllDates)
                .then((cond) => closedTreasure_bloc.instance.add(getListClosedTreasure_Event(conditions: cond)));
          },
          icon: Icon(
  ...
  void deleteItem(Fina_ClosedTreasure item) {
          ...
          Row(
            children: [
              ElevatedButton.icon(
                ...
                onPressed: () {
                  bllFina_ClosedTreasure.fire_DeleteItem(item.ID.toString());
                  loadDataFromDB();
                  Navigator.of(context).pop();
                },
              ),
  ...
  void shareItem(Fina_ClosedTreasure item) async {
