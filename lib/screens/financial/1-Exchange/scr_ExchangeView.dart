...
import 'package:vigil_erp/screens/financial/1-Exchange/scr_ExchangeItem.dart';
...
class scr_ExchangeView extends StatefulWidget {
  scr_ExchangeView({super.key});

  @override
  State<scr_ExchangeView> createState() => _scr_ExchangeViewState();
}

class _scr_ExchangeViewState extends State<scr_ExchangeView> {
        ...
        child: Column(
          children: [
            Center(
              ...
              children: [
                ctr_TextHeaderPage(
                  text: 'عرض سندات الدفع',
                  style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 25),
                  color: Colors.grey[300],
                  borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
                ),
                BlocBuilder<exchange_bloc, financial_state>(
                  builder: (context, state) {
                    if (state is getListExchange_StateDataChanged)
                      return ctr_TextHeaderPage(
                        text: state.filterdLst_Exchange.length.toString(),
                        color: Colors.grey[300],
                        borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
              ...
              child: Row(
                children: [
                  Expanded(
                    ...
                    OnChanged: (value) {
                      if (value != null) {
                        exchange_bloc.instance.add(filterAnyExchange_Event(filterData: value.trim()));
                      }
                      return null;
                    },
                  )),
                  IconButton(
                    onPressed: () {
                      ...
                      exchange_bloc.instance.add(resetFilterExchange_Event());
                    },
  ...
  Widget buildListView(BuildContext context) {
    return BlocBuilder<exchange_bloc, financial_state>(
      builder: (context, state) {
        if (state is financialnitial) {
          return const Center(child: CircularProgressIndicator());
        } else if (state is getListExchange_StateDataChanged) {
          return SingleChildScrollView(
                    ...
                    child: SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          return buildListViewItem(state.filterdLst_Exchange[index], context);
                        },
                        separatorBuilder: (context, index) => const SizedBox(height: 1),
                        itemCount: state.filterdLst_Exchange.length,
                      ),
                    ),
                  ),
                      ...
                      children: [
                        ...
                        BlocBuilder<exchange_bloc, financial_state>(
                          builder: (context, state) {
                            if (state is getListExchange_StateDataChanged) {
                              contTotalValue.text =
                                  state.filterdLst_Exchange.fold(0.0, (previousValue, element) => previousValue + element.Value!).toStringAsFixed(2);
                              return SizedBox(
                                width: 200,
                                child: ctr_TextFormField(
  ...
  buildListViewItem(Fina_Exchange item, context) {
    return Column(
      children: [
        Divider(
          color: Colors.grey,
  ...
  Future loadDataFromDB() async {
    ...
    await financialCluses_bloc.instance.getLstFinancialAsDataSource(condions: [
      ...
      BLLCondions(enTable_Def_FinancialCluses.IDFinancialType.name, en_CondionsWhere.isEqualTo, en_finacialType.finacialTypeExchange.value),
    ]);
    // await exchangeType_bloc.instance.add(getListExchangeTypeAsDataSource_Event());
    ...
    exchange_bloc.instance.add(getListExchange_Event());
    if (controllerfilter.text.trim().isNotEmpty) {
      exchange_bloc.instance.add(filterAnyExchange_Event(filterData: controllerfilter.text.trim()));
    }
  }
  ...
  void newItem() async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_ExchangeItem(null, scaffold, en_FormMode.NewMode),
      ),
    );
  ...
  void editItem(Fina_Exchange item) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_ExchangeItem(item, scaffold, en_FormMode.EditMode),
      ),
    );
    ...
    return Row(
      children: [
        IconButton(
          onPressed: () {
            sharedControls
                .showFormFilterByDates(context, en_TablesName.Fina_Exchange, branchID, contDateFrom, contDateTo, isGetAllDates)
                .then((retValues) {
              if (retValues != null) {
        ...
        IconButton(
          onPressed: () {
            tablesCondions
                ...
                .then((cond) => exchange_bloc.instance.add(getListExchange_Event(conditions: cond)));
          },
          icon: Icon(
  ...
  void deleteItem(Fina_Exchange item) {
          ...
          Row(
            children: [
              ElevatedButton.icon(
                ...
                onPressed: () {
                  bllFina_Exchange.fire_DeleteItem(item.ID.toString());
                  loadDataFromDB();
                  Navigator.of(context).pop();
                },
              ),
  ...
  void shareItem(Fina_Exchange item) async {
