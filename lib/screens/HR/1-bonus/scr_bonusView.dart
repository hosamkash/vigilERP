...
import 'package:vigil_erp/screens/HR/1-bonus/scr_bonusItem.dart';
import 'package:vigil_erp/shared/enumerators.dart';
import 'package:vigil_erp/shared/shared_controls.dart';
import '../../../bll/bllFirebase/bllHR_Bonus.dart';
import '../../../bll/classModel/HR_Bonus.dart';
import '../../../blocManagment/blocHR/hr_bloc.dart';

class scr_bonusView extends StatefulWidget {
  scr_bonusView({super.key});

  @override
  State<scr_bonusView> createState() => _scr_bonusViewState();
}

class _scr_bonusViewState extends State<scr_bonusView> {
          ...
          children: [
            Center(
              ...
              children: [
                ...
                BlocBuilder<bonus_bloc, hr_state>(
                  builder: (context, state) {
                    if (state is getListBonus_StateDataChanged)
                      return ctr_TextHeaderPage(
                        text: state.filterdLst_Bonus.length.toString(),
                        color: Colors.grey[300],
                        borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
              ...
              child: Row(
                children: [
                  Expanded(
                    ...
                    OnChanged: (value) {
                      if (value != null) {
                        bonus_bloc.instance.add(filterAnyBonus_Event(filterData: value.trim()));
                      }
                      return null;
                    },
                  )),
                  IconButton(
                    onPressed: () {
                      ...
                      bonus_bloc.instance.add(resetFilterBonus_Event());
                    },
  ...
  Widget buildListView(BuildContext context) {
    return BlocBuilder<bonus_bloc, hr_state>(
      builder: (context, state) {
        if (state is hr_StateInitial) {
          return const Center(child: CircularProgressIndicator());
        } else if (state is getListBonus_StateDataChanged) {
          return SingleChildScrollView(
                    ...
                    child: SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          return buildListViewItem(state.filterdLst_Bonus[index], context);
                        },
                        separatorBuilder: (context, index) => const SizedBox(height: 1),
                        itemCount: state.filterdLst_Bonus.length,
                      ),
                    ),
                  ),
                  SizedBox(
                    ...
                    child: Row(
                      children: [
                        ...
                        BlocBuilder<bonus_bloc, hr_state>(
                          builder: (context, state) {
                            if (state is getListBonus_StateDataChanged) {
                              contTotalValue.text = state.filterdLst_Bonus
                                  .fold(0.0, (previousValue, element) => previousValue + element.BounsValue!)
                                  .toStringAsFixed(2);
                              return SizedBox(
                                width: 200,
                                child: ctr_TextFormField(
  ...
  buildListViewItem(HR_Bonus item, context) {
    return Column(
      children: [
        Divider(
          color: Colors.grey,
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                SizedBox(
                  width: 120,
                  child: Text('${item.BounsDays} ${bonusType_bloc.instance.getNameByID(item.BounsType)} = ${item.BounsValue}', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                ),
                SizedBox(
                  width: 200,
                  child: Text(item.BounsReson! , textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                ),
                SizedBox(
                  width: 40,
  ...
  Future loadDataFromDB() async {
    ...
    await bonusType_bloc.instance.getLst_bonusType();
    // bonusType_bloc.instance.add(getListBonusTypeAsDataSource_Event());

    bonus_bloc.instance.add(getListBonus_Event());
    if (controllerfilter.text.trim().isNotEmpty) {
      bonus_bloc.instance.add(filterAnyBonus_Event(filterData: controllerfilter.text.trim()));
    }
  }
  ...
  void newItem() async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_bonusItem(null, scaffold, en_FormMode.NewMode),
      ),
    );
  ...
  void editItem(HR_Bonus item) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_bonusItem(item, scaffold, en_FormMode.EditMode),
      ),
    );
    ...
    return Row(
      children: [
        IconButton(
          onPressed: () {
            sharedControls
                .showFormFilterByDates(context, en_TablesName.HR_Bonus, branchID, contDateFrom, contDateTo, isGetAllDates)
                .then((retValues) {
              if (retValues != null) {
        ...
        IconButton(
          onPressed: () {
            tablesCondions
                ...
                .then((cond) => bonus_bloc.instance.add(getListBonus_Event(conditions: cond)));
          },
          icon: Icon(
          ...
          Row(
            children: [
              ElevatedButton.icon(
                ...
                onPressed: () {
                  bllHR_Bonus.fire_DeleteItem(item.ID.toString());
                  loadDataFromDB();
                  Navigator.of(context).pop();
                },
              ),
  ...
  void shareItem(HR_Bonus item) async {
