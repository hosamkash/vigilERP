...
import 'package:vigil_erp/screens/HR/2-discount/scr_discountItem.dart';
...
class scr_discountView extends StatefulWidget {
  scr_discountView({super.key});

  @override
  State<scr_discountView> createState() => _scr_discountViewState();
}

class _scr_discountViewState extends State<scr_discountView> {
        ...
        child: Column(
          children: [
            Center(
              ...
              children: [
                ctr_TextHeaderPage(
                  text: 'الجزاءات والخصومات',
                  style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 25),
                  color: Colors.grey[300],
                  borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
                ),
                BlocBuilder<discount_bloc, hr_state>(
                  builder: (context, state) {
                    if (state is getListDiscount_StateDataChanged)
                      return ctr_TextHeaderPage(
                        text: state.filterdLst_Discount.length.toString(),
                        color: Colors.grey[300],
                        borderRadius: const BorderRadiusDirectional.all(Radius.circular(10)),
              ...
              child: Row(
                children: [
                  Expanded(
                    ...
                    OnChanged: (value) {
                      if (value != null) {
                        discount_bloc.instance.add(filterAnyDiscount_Event(filterData: value.trim()));
                      }
                      return null;
                    },
                  )),
                  IconButton(
                    onPressed: () {
                      ...
                      discount_bloc.instance.add(resetFilterDiscount_Event());
                    },
  ...
  Widget buildListView(BuildContext context) {
    return BlocBuilder<discount_bloc, hr_state>(
      builder: (context, state) {
        if (state is hr_StateInitial) {
          return const Center(child: CircularProgressIndicator());
        } else if (state is getListDiscount_StateDataChanged) {
          return SingleChildScrollView(
                    ...
                    child: SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          return buildListViewItem(state.filterdLst_Discount[index], context);
                        },
                        separatorBuilder: (context, index) => const SizedBox(height: 1),
                        itemCount: state.filterdLst_Discount.length,
                      ),
                    ),
                  ),
                  SizedBox(
                    ...
                    child: Row(
                      children: [
                        ...
                        BlocBuilder<discount_bloc, hr_state>(
                          builder: (context, state) {
                            if (state is getListDiscount_StateDataChanged) {
                              contTotalValue.text = state.filterdLst_Discount
  ...
  buildListViewItem(HR_Discount item, context) {
    return Column(
      children: [
        Divider(
          color: Colors.grey,
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                SizedBox(
                  width: 120,
                  child: Text('${item.DiscountDays} ${bonusType_bloc.instance.getNameByID(item.DiscountType)} = ${item.DiscountValue}', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                ),
                SizedBox(
                  width: 200,
                  child: Text(item.DiscountReson! , textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                ),
                SizedBox(
                  width: 40,
  ...
  Future loadDataFromDB() async {
    ...
    await bonusType_bloc.instance.getLst_bonusTypeAsDataSource();
    employee_bloc.instance.add(getLstEmployeeAsDataSource_Event());
    // discountType_bloc.instance.add(getListDiscountTypeAsDataSource_Event());

    discount_bloc.instance.add(getListDiscount_Event());
    if (controllerfilter.text.trim().isNotEmpty) {
      discount_bloc.instance.add(filterAnyDiscount_Event(filterData: controllerfilter.text.trim()));
    }
  }
  ...
  void newItem() async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_discountItem(null, scaffold, en_FormMode.NewMode),
      ),
    );
  ...
  void editItem(HR_Discount item) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => scr_discountItem(item, scaffold, en_FormMode.EditMode),
      ),
    );
    ...
    return Row(
      children: [
        IconButton(
          onPressed: () {
            sharedControls
                .showFormFilterByDates(context, en_TablesName.HR_Discount, branchID, contDateFrom, contDateTo, isGetAllDates)
                .then((retValues) {
              if (retValues != null) {
        ...
        IconButton(
          onPressed: () {
            tablesCondions
                ...
                .then((cond) => discount_bloc.instance.add(getListDiscount_Event(conditions: cond)));
          },
          icon: Icon(
  ...
  void deleteItem(HR_Discount item) {
          ...
          Row(
            children: [
              ElevatedButton.icon(
                ...
                onPressed: () {
                  bllHR_Discount.fire_DeleteItem(item.ID.toString());
                  loadDataFromDB();
                  Navigator.of(context).pop();
                },
              ),
  ...
  void shareItem(HR_Discount item) async {
