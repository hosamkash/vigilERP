import '../classModel/Fix_MilitaryStatus.dart';
import 'dart:io';
...
class bllFix_MilitaryStatus
	{
		static List<String> ColumnsName = enTable_Fix_MilitaryStatus.values.map((item) => item.name).toList();
			static late List<Fix_MilitaryStatus> lstFix_MilitaryStatus = [];
			static late Fix_MilitaryStatus itemFix_MilitaryStatus;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_MilitaryStatus.name)
	.orderBy(enTable_Fix_MilitaryStatus.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_MilitaryStatus.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_MilitaryStatus colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_MilitaryStatus  itemFix_MilitaryStatus) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name).add(itemFix_MilitaryStatus.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_MilitaryStatus) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name).doc(docName).set(itemFix_MilitaryStatus).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_MilitaryStatus  itemFix_MilitaryStatus) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name).doc(docName).set(itemFix_MilitaryStatus.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_MilitaryStatus  itemFix_MilitaryStatus,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_MilitaryStatus.toMap());
...
static Future fire_setListMaster({
	required List<Fix_MilitaryStatus> lstFix_MilitaryStatus,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_MilitaryStatus.forEach((elm){
...
static Future<Fix_MilitaryStatus> fire_getItem(String docName) async {
	late Fix_MilitaryStatus  itemFix_MilitaryStatus;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name).doc(docName).get();
	 itemFix_MilitaryStatus = Fix_MilitaryStatus();
 itemFix_MilitaryStatus.ID = retValue.data()!['ID']; itemFix_MilitaryStatus.Name = retValue.data()!['Name'];	return  itemFix_MilitaryStatus;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_MilitaryStatus>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_MilitaryStatus.name)
	.get()
	.then((value) {
	lstFix_MilitaryStatus.clear();
	value.docs.forEach((element) {
	lstFix_MilitaryStatus.add(Fix_MilitaryStatus.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_MilitaryStatus;
}

static Future<List<Fix_MilitaryStatus>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_MilitaryStatus.clear();value.docs.forEach((element) {
	lstFix_MilitaryStatus.add(Fix_MilitaryStatus.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_MilitaryStatus;
}

static Future<List<Fix_MilitaryStatus>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_MilitaryStatus.add(Fix_MilitaryStatus.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_MilitaryStatus;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MilitaryStatus.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_MilitaryStatus.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_MilitaryStatus.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
