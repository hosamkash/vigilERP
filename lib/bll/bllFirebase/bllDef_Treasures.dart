import '../classModel/Def_Treasures.dart';
import 'dart:io';
...
class bllDef_Treasures
	{
		static List<String> ColumnsName = enTable_Def_Treasures.values.map((item) => item.name).toList();
			static late List<Def_Treasures> lstDef_Treasures = [];
			static late Def_Treasures itemDef_Treasures;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Def_Treasures.name)
	.orderBy(enTable_Def_Treasures.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Def_Treasures.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Def_Treasures colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Def_Treasures  itemDef_Treasures) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name).add(itemDef_Treasures.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemDef_Treasures) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name).doc(docName).set(itemDef_Treasures).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Def_Treasures  itemDef_Treasures) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name).doc(docName).set(itemDef_Treasures.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Def_Treasures  itemDef_Treasures,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemDef_Treasures.toMap());
...
static Future fire_setListMaster({
	required List<Def_Treasures> lstDef_Treasures,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstDef_Treasures.forEach((elm){
...
static Future<Def_Treasures> fire_getItem(String docName) async {
	late Def_Treasures  itemDef_Treasures;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name).doc(docName).get();
	 itemDef_Treasures = Def_Treasures();
 itemDef_Treasures.ID = retValue.data()!['ID']; itemDef_Treasures.Code = retValue.data()!['Code']; itemDef_Treasures.Name = retValue.data()!['Name']; itemDef_Treasures.IsActive = retValue.data()!['IsActive']; itemDef_Treasures.IsBindBranch = retValue.data()!['IsBindBranch']; itemDef_Treasures.IDBranch = retValue.data()!['IDBranch']; itemDef_Treasures.Balance = retValue.data()!['Balance'];	return  itemDef_Treasures;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Def_Treasures>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Def_Treasures.name)
	.get()
	.then((value) {
	lstDef_Treasures.clear();
	value.docs.forEach((element) {
	lstDef_Treasures.add(Def_Treasures.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstDef_Treasures;
}

static Future<List<Def_Treasures>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstDef_Treasures.clear();value.docs.forEach((element) {
	lstDef_Treasures.add(Def_Treasures.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstDef_Treasures;
}

static Future<List<Def_Treasures>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstDef_Treasures.add(Def_Treasures.fromJson(element.data() as Map<String, dynamic> )); });
	return lstDef_Treasures;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Treasures.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Def_Treasures.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Def_Treasures.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
