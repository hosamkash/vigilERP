import '../classModel/Fix_Gender.dart';
import 'dart:io';
...
class bllFix_Gender
	{
		static List<String> ColumnsName = enTable_Fix_Gender.values.map((item) => item.name).toList();
			static late List<Fix_Gender> lstFix_Gender = [];
			static late Fix_Gender itemFix_Gender;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_Gender.name)
	.orderBy(enTable_Fix_Gender.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_Gender.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_Gender colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_Gender  itemFix_Gender) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name).add(itemFix_Gender.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_Gender) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name).doc(docName).set(itemFix_Gender).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_Gender  itemFix_Gender) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name).doc(docName).set(itemFix_Gender.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_Gender  itemFix_Gender,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_Gender.toMap());
...
static Future fire_setListMaster({
	required List<Fix_Gender> lstFix_Gender,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_Gender.forEach((elm){
...
static Future<Fix_Gender> fire_getItem(String docName) async {
	late Fix_Gender  itemFix_Gender;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name).doc(docName).get();
	 itemFix_Gender = Fix_Gender();
 itemFix_Gender.ID = retValue.data()!['ID']; itemFix_Gender.Name = retValue.data()!['Name'];	return  itemFix_Gender;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_Gender>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_Gender.name)
	.get()
	.then((value) {
	...
	lstFix_Gender.add(Fix_Gender.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_Gender;
}

static Future<List<Fix_Gender>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_Gender.clear();value.docs.forEach((element) {
	lstFix_Gender.add(Fix_Gender.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_Gender;
}

static Future<List<Fix_Gender>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_Gender.add(Fix_Gender.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_Gender;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Gender.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_Gender.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_Gender.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
