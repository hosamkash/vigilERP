import '../classModel/Def_Units.dart';
import 'dart:io';
...
class bllDef_Units
	{
		static List<String> ColumnsName = enTable_Def_Units.values.map((item) => item.name).toList();
			static late List<Def_Units> lstDef_Units = [];
			static late Def_Units itemDef_Units;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Def_Units.name)
	.orderBy(enTable_Def_Units.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Def_Units.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Def_Units colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Def_Units  itemDef_Units) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name).add(itemDef_Units.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemDef_Units) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name).doc(docName).set(itemDef_Units).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Def_Units  itemDef_Units) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name).doc(docName).set(itemDef_Units.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Def_Units  itemDef_Units,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemDef_Units.toMap());
...
static Future fire_setListMaster({
	required List<Def_Units> lstDef_Units,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstDef_Units.forEach((elm){
...
static Future<Def_Units> fire_getItem(String docName) async {
	late Def_Units  itemDef_Units;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name).doc(docName).get();
	 itemDef_Units = Def_Units();
 itemDef_Units.ID = retValue.data()!['ID']; itemDef_Units.Name = retValue.data()!['Name']; itemDef_Units.IsActive = retValue.data()!['IsActive'];	return  itemDef_Units;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Def_Units>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Def_Units.name)
	.get()
	.then((value) {
	...
	lstDef_Units.add(Def_Units.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
...
static Future<List<Def_Units>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstDef_Units.clear();value.docs.forEach((element) {
	lstDef_Units.add(Def_Units.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstDef_Units;
}

static Future<List<Def_Units>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstDef_Units.add(Def_Units.fromJson(element.data() as Map<String, dynamic> )); });
	return lstDef_Units;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Units.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Def_Units.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Def_Units.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
