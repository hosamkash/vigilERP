import '../classModel/Fix_Religions.dart';
import 'dart:io';
...
class bllFix_Religions
	{
		static List<String> ColumnsName = enTable_Fix_Religions.values.map((item) => item.name).toList();
			static late List<Fix_Religions> lstFix_Religions = [];
			static late Fix_Religions itemFix_Religions;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_Religions.name)
	.orderBy(enTable_Fix_Religions.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_Religions.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_Religions colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_Religions  itemFix_Religions) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name).add(itemFix_Religions.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_Religions) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name).doc(docName).set(itemFix_Religions).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_Religions  itemFix_Religions) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name).doc(docName).set(itemFix_Religions.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_Religions  itemFix_Religions,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_Religions.toMap());
...
static Future fire_setListMaster({
	required List<Fix_Religions> lstFix_Religions,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_Religions.forEach((elm){
...
static Future<Fix_Religions> fire_getItem(String docName) async {
	late Fix_Religions  itemFix_Religions;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name).doc(docName).get();
	 itemFix_Religions = Fix_Religions();
 itemFix_Religions.ID = retValue.data()!['ID']; itemFix_Religions.Name = retValue.data()!['Name'];	return  itemFix_Religions;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_Religions>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_Religions.name)
	.get()
	.then((value) {
	lstFix_Religions.clear();
	value.docs.forEach((element) {
	lstFix_Religions.add(Fix_Religions.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_Religions;
}

static Future<List<Fix_Religions>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_Religions.clear();value.docs.forEach((element) {
	lstFix_Religions.add(Fix_Religions.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_Religions;
}

static Future<List<Fix_Religions>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_Religions.add(Fix_Religions.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_Religions;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Religions.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_Religions.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_Religions.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
