import '../classModel/Fix_DocumentsType.dart';
import 'dart:io';
...
class bllFix_DocumentsType
	{
		static List<String> ColumnsName = enTable_Fix_DocumentsType.values.map((item) => item.name).toList();
			static late List<Fix_DocumentsType> lstFix_DocumentsType = [];
			static late Fix_DocumentsType itemFix_DocumentsType;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_DocumentsType.name)
	.orderBy(enTable_Fix_DocumentsType.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_DocumentsType.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_DocumentsType colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_DocumentsType  itemFix_DocumentsType) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name).add(itemFix_DocumentsType.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_DocumentsType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name).doc(docName).set(itemFix_DocumentsType).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_DocumentsType  itemFix_DocumentsType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name).doc(docName).set(itemFix_DocumentsType.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_DocumentsType  itemFix_DocumentsType,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_DocumentsType.toMap());
...
static Future fire_setListMaster({
	required List<Fix_DocumentsType> lstFix_DocumentsType,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_DocumentsType.forEach((elm){
...
static Future<Fix_DocumentsType> fire_getItem(String docName) async {
	late Fix_DocumentsType  itemFix_DocumentsType;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name).doc(docName).get();
	 itemFix_DocumentsType = Fix_DocumentsType();
 itemFix_DocumentsType.ID = retValue.data()!['ID']; itemFix_DocumentsType.Name = retValue.data()!['Name'];	return  itemFix_DocumentsType;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_DocumentsType>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_DocumentsType.name)
	.get()
	.then((value) {
	lstFix_DocumentsType.clear();
	value.docs.forEach((element) {
	lstFix_DocumentsType.add(Fix_DocumentsType.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_DocumentsType;
}

static Future<List<Fix_DocumentsType>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_DocumentsType.clear();value.docs.forEach((element) {
	lstFix_DocumentsType.add(Fix_DocumentsType.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_DocumentsType;
}

static Future<List<Fix_DocumentsType>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_DocumentsType.add(Fix_DocumentsType.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_DocumentsType;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_DocumentsType.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_DocumentsType.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_DocumentsType.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
