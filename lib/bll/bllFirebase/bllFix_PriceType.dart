import '../classModel/Fix_PriceType.dart';
import 'dart:io';
...
class bllFix_PriceType
	{
		static List<String> ColumnsName = enTable_Fix_PriceType.values.map((item) => item.name).toList();
			static late List<Fix_PriceType> lstFix_PriceType = [];
			static late Fix_PriceType itemFix_PriceType;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_PriceType.name)
	.orderBy(enTable_Fix_PriceType.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_PriceType.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_PriceType colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_PriceType  itemFix_PriceType) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name).add(itemFix_PriceType.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_PriceType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name).doc(docName).set(itemFix_PriceType).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_PriceType  itemFix_PriceType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name).doc(docName).set(itemFix_PriceType.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_PriceType  itemFix_PriceType,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_PriceType.toMap());
...
static Future fire_setListMaster({
	required List<Fix_PriceType> lstFix_PriceType,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_PriceType.forEach((elm){
...
static Future<Fix_PriceType> fire_getItem(String docName) async {
	late Fix_PriceType  itemFix_PriceType;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name).doc(docName).get();
	 itemFix_PriceType = Fix_PriceType();
 itemFix_PriceType.ID = retValue.data()!['ID']; itemFix_PriceType.Name = retValue.data()!['Name'];	return  itemFix_PriceType;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_PriceType>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_PriceType.name)
	.get()
	.then((value) {
	lstFix_PriceType.clear();
	value.docs.forEach((element) {
	lstFix_PriceType.add(Fix_PriceType.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_PriceType;
}

static Future<List<Fix_PriceType>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_PriceType.clear();value.docs.forEach((element) {
	lstFix_PriceType.add(Fix_PriceType.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_PriceType;
}

static Future<List<Fix_PriceType>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_PriceType.add(Fix_PriceType.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_PriceType;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_PriceType.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_PriceType.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_PriceType.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
