import '../classModel/Fix_Address_City.dart';
import 'dart:io';
...
class bllFix_Address_City
	{
		static List<String> ColumnsName = enTable_Fix_Address_City.values.map((item) => item.name).toList();
			static late List<Fix_Address_City> lstFix_Address_City = [];
			static late Fix_Address_City itemFix_Address_City;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_Address_City.name)
	.orderBy(enTable_Fix_Address_City.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_Address_City.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_Address_City colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_Address_City  itemFix_Address_City) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name).add(itemFix_Address_City.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_Address_City) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name).doc(docName).set(itemFix_Address_City).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_Address_City  itemFix_Address_City) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name).doc(docName).set(itemFix_Address_City.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_Address_City  itemFix_Address_City,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_Address_City.toMap());
...
static Future fire_setListMaster({
	required List<Fix_Address_City> lstFix_Address_City,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_Address_City.forEach((elm){
...
static Future<Fix_Address_City> fire_getItem(String docName) async {
	late Fix_Address_City  itemFix_Address_City;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name).doc(docName).get();
	 itemFix_Address_City = Fix_Address_City();
 itemFix_Address_City.ID = retValue.data()!['ID']; itemFix_Address_City.Name = retValue.data()!['Name']; itemFix_Address_City.IDGovernment = retValue.data()!['IDGovernment'];	return  itemFix_Address_City;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_Address_City>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_Address_City.name)
	.get()
	.then((value) {
	lstFix_Address_City.clear();
	value.docs.forEach((element) {
	lstFix_Address_City.add(Fix_Address_City.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_Address_City;
}

static Future<List<Fix_Address_City>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_Address_City.clear();value.docs.forEach((element) {
	lstFix_Address_City.add(Fix_Address_City.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_Address_City;
}

static Future<List<Fix_Address_City>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_Address_City.add(Fix_Address_City.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_Address_City;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_City.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_Address_City.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_Address_City.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
