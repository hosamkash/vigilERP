import '../classModel/Fix_RequestStatus.dart';
import 'dart:io';
...
class bllFix_RequestStatus
	{
		static List<String> ColumnsName = enTable_Fix_RequestStatus.values.map((item) => item.name).toList();
			static late List<Fix_RequestStatus> lstFix_RequestStatus = [];
			static late Fix_RequestStatus itemFix_RequestStatus;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_RequestStatus.name)
	.orderBy(enTable_Fix_RequestStatus.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_RequestStatus.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_RequestStatus colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_RequestStatus  itemFix_RequestStatus) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name).add(itemFix_RequestStatus.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_RequestStatus) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name).doc(docName).set(itemFix_RequestStatus).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_RequestStatus  itemFix_RequestStatus) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name).doc(docName).set(itemFix_RequestStatus.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_RequestStatus  itemFix_RequestStatus,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_RequestStatus.toMap());
...
static Future fire_setListMaster({
	required List<Fix_RequestStatus> lstFix_RequestStatus,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_RequestStatus.forEach((elm){
...
static Future<Fix_RequestStatus> fire_getItem(String docName) async {
	late Fix_RequestStatus  itemFix_RequestStatus;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name).doc(docName).get();
	 itemFix_RequestStatus = Fix_RequestStatus();
 itemFix_RequestStatus.ID = retValue.data()!['ID']; itemFix_RequestStatus.Name = retValue.data()!['Name'];	return  itemFix_RequestStatus;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_RequestStatus>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_RequestStatus.name)
	.get()
	.then((value) {
	lstFix_RequestStatus.clear();
	value.docs.forEach((element) {
	lstFix_RequestStatus.add(Fix_RequestStatus.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_RequestStatus;
}

static Future<List<Fix_RequestStatus>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_RequestStatus.clear();value.docs.forEach((element) {
	lstFix_RequestStatus.add(Fix_RequestStatus.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_RequestStatus;
}

static Future<List<Fix_RequestStatus>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_RequestStatus.add(Fix_RequestStatus.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_RequestStatus;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_RequestStatus.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_RequestStatus.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_RequestStatus.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
