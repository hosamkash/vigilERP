import '../classModel/Def_Categories.dart';
import 'dart:io';
...
class bllDef_Categories {
  static List<String> ColumnsName = enTable_Def_Categories.values.map((item) => item.name).toList();
  static late List<Def_Categories> lstDef_Categories = [];
  static late Def_Categories itemDef_Categories;
  static String query = '';
...
//**************************** general function ****************************
  ...
  static Future<int> getMaxID_firestore() async {
    ...
    await FirebaseFirestore.instance
        .collection(en_TablesName.Def_Categories.name)
        .orderBy(enTable_Def_Categories.ID.name, descending: true)
        .limit(1)
        .get()
        .then((value) {
      if (value.docs.isEmpty) {
        ...
        maxID = value.docs.first.get(enTable_Def_Categories.ID.name);
        maxID += 1;
      }
    }).catchError((error) {
  ...
  static Future<int> getMax_firestore(enTable_Def_Categories colName, {BLLCondions? condion}) async {
    int maxID = 1;
    ...
    try {
      if (condion != null) {
        ...
        snapshot =
            await FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name).orderBy(colName.name, descending: true).limit(1).get();
      }
...
//**************************** add & set ****************************
  ...
  static Future<String> fire_AddItem(Def_Categories itemDef_Categories) async {
    String docCreatedName = '';
    await FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name).add(itemDef_Categories.toMap()).then((val) {
  ...
  static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic> itemDef_Categories) async {
    bool result = false;
    await FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name).doc(docName).set(itemDef_Categories).then((val) {
      result = true;
    }).catchError((error) {
      print(error.toString());
      throw error;
  ...
  static Future<bool> fire_SetItem(String docName, Def_Categories itemDef_Categories) async {
    bool result = false;
    await FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name).doc(docName).set(itemDef_Categories.toMap()).then((val) {
      result = true;
    }).catchError((error) {
      print(error.toString());
      throw error;
  ...
  static Future fire_setListMaster_And_Details({
    ...
    required Def_Categories itemDef_Categories,
    ...
    CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name);
    DocumentReference tablemasterDocref = coll.doc(insertdDocID);
    WriteBatch batch = FirebaseFirestore.instance.batch();
    // save Master
    batch.set(tablemasterDocref, itemDef_Categories.toMap());
  ...
  static Future fire_setListMaster({
    required List<Def_Categories> lstDef_Categories,
  }) async {
    CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name);
    WriteBatch batch = FirebaseFirestore.instance.batch();
    lstDef_Categories.forEach((elm) {
...
//**************************** get ****************************
  ...
  static Future<Def_Categories> fire_getItem(String docName) async {
    late Def_Categories itemDef_Categories;
    try {
      var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name).doc(docName).get();
      itemDef_Categories = Def_Categories();
      itemDef_Categories.ID = retValue.data()!['ID'];
      itemDef_Categories.Name = retValue.data()!['Name'];
      itemDef_Categories.IsActive = retValue.data()!['IsActive'];
      itemDef_Categories.IsSalesCategory = retValue.data()!['IsSalesCategory'];
      itemDef_Categories.IsViewAllProducts = retValue.data()!['IsViewAllProducts'];
      return itemDef_Categories;
    } catch (error) {
      print(error.toString());
      throw error;
    }
  ...
  static Future<List<Def_Categories>> fire_getList() async {
    await FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name).get().then((value) {
      lstDef_Categories.clear();
      value.docs.forEach((element) {
        lstDef_Categories.add(Def_Categories.fromJson(element.data()));
      });
    }).catchError((error) {
      print(error.toString());
      throw error;
    });
    return lstDef_Categories;
  }

  static Future<List<Def_Categories>> fire_getListByBranch(int branchID, String BranchColumnName) async {
    await FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name).where(BranchColumnName, isEqualTo: branchID).get().then((value) {
      lstDef_Categories.clear();
      value.docs.forEach((element) {
        lstDef_Categories.add(Def_Categories.fromJson(element.data()));
      });
  ...
  static Future<List<Def_Categories>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
    CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name);
    Query query = colRef;
    if (conditions != null) {
    ...
    snap.docs.forEach((element) {
      lstDef_Categories.add(Def_Categories.fromJson(element.data() as Map<String, dynamic>));
    });
    return lstDef_Categories;
  }
...
//**************************** delete ****************************
  ...
  static Future<bool> fire_DeleteItem(String docName) async {
    bool result = false;
    await FirebaseFirestore.instance.collection(en_TablesName.Def_Categories.name).doc(docName).delete().then((val) {
      result = true;
    }).catchError((error) {
      print(error.toString());
      result = false;
...
//**************************** Upload Files ****************************
  ...
  static Future<String> storage_UploadFile(String fileName, File file, {String pathOnStorage = ''}) async {
    pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Def_Categories.name : pathOnStorage;
  ...
  static Future<String> storage_GetFileDowenloadURL(String fileName) async {
    String fileDownloadURL = '';
    await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Def_Categories.name}/${fileName}').getDownloadURL().then((URL) {
      fileDownloadURL = URL;
    }).catchError((error) {
      print(error.toString());
      throw error;
