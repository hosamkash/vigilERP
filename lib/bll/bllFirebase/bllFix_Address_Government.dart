import '../classModel/Fix_Address_Government.dart';
import 'dart:io';
...
class bllFix_Address_Government
	{
		static List<String> ColumnsName = enTable_Fix_Address_Government.values.map((item) => item.name).toList();
			static late List<Fix_Address_Government> lstFix_Address_Government = [];
			static late Fix_Address_Government itemFix_Address_Government;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_Address_Government.name)
	.orderBy(enTable_Fix_Address_Government.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_Address_Government.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_Address_Government colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_Address_Government  itemFix_Address_Government) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name).add(itemFix_Address_Government.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_Address_Government) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name).doc(docName).set(itemFix_Address_Government).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_Address_Government  itemFix_Address_Government) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name).doc(docName).set(itemFix_Address_Government.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_Address_Government  itemFix_Address_Government,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_Address_Government.toMap());
...
static Future fire_setListMaster({
	required List<Fix_Address_Government> lstFix_Address_Government,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_Address_Government.forEach((elm){
...
static Future<Fix_Address_Government> fire_getItem(String docName) async {
	late Fix_Address_Government  itemFix_Address_Government;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name).doc(docName).get();
	 itemFix_Address_Government = Fix_Address_Government();
 itemFix_Address_Government.ID = retValue.data()!['ID']; itemFix_Address_Government.Name = retValue.data()!['Name'];	return  itemFix_Address_Government;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_Address_Government>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_Address_Government.name)
	.get()
	.then((value) {
	lstFix_Address_Government.clear();
	value.docs.forEach((element) {
	lstFix_Address_Government.add(Fix_Address_Government.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_Address_Government;
}

static Future<List<Fix_Address_Government>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_Address_Government.clear();value.docs.forEach((element) {
	lstFix_Address_Government.add(Fix_Address_Government.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_Address_Government;
}

static Future<List<Fix_Address_Government>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_Address_Government.add(Fix_Address_Government.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_Address_Government;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Address_Government.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_Address_Government.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_Address_Government.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
