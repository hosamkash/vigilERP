import '../classModel/Dealing_Vendors.dart';
import 'dart:io';
...
class bllDealing_Vendors
	{
		static List<String> ColumnsName = enTable_Dealing_Vendors.values.map((item) => item.name).toList();
			static late List<Dealing_Vendors> lstDealing_Vendors = [];
			static late Dealing_Vendors itemDealing_Vendors;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Dealing_Vendors.name)
	.orderBy(enTable_Dealing_Vendors.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Dealing_Vendors.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Dealing_Vendors colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Dealing_Vendors  itemDealing_Vendors) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name).add(itemDealing_Vendors.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemDealing_Vendors) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name).doc(docName).set(itemDealing_Vendors).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Dealing_Vendors  itemDealing_Vendors) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name).doc(docName).set(itemDealing_Vendors.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Dealing_Vendors  itemDealing_Vendors,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemDealing_Vendors.toMap());
...
static Future fire_setListMaster({
	required List<Dealing_Vendors> lstDealing_Vendors,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstDealing_Vendors.forEach((elm){
...
static Future<Dealing_Vendors> fire_getItem(String docName) async {
	late Dealing_Vendors  itemDealing_Vendors;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name).doc(docName).get();
	 itemDealing_Vendors = Dealing_Vendors();
 itemDealing_Vendors.ID = retValue.data()!['ID']; itemDealing_Vendors.IDBranch = retValue.data()!['IDBranch']; itemDealing_Vendors.Code = retValue.data()!['Code']; itemDealing_Vendors.Name = retValue.data()!['Name']; itemDealing_Vendors.IsActive = retValue.data()!['IsActive']; itemDealing_Vendors.CurrentBalance = retValue.data()!['CurrentBalance']; itemDealing_Vendors.BalanceType = retValue.data()!['BalanceType']; itemDealing_Vendors.Mobile = retValue.data()!['Mobile']; itemDealing_Vendors.Phone = retValue.data()!['Phone']; itemDealing_Vendors.Address = retValue.data()!['Address']; itemDealing_Vendors.Note = retValue.data()!['Note'];	return  itemDealing_Vendors;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Dealing_Vendors>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Dealing_Vendors.name)
	.get()
	.then((value) {
	lstDealing_Vendors.clear();
	value.docs.forEach((element) {
	lstDealing_Vendors.add(Dealing_Vendors.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstDealing_Vendors;
}

static Future<List<Dealing_Vendors>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstDealing_Vendors.clear();value.docs.forEach((element) {
	lstDealing_Vendors.add(Dealing_Vendors.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstDealing_Vendors;
}

static Future<List<Dealing_Vendors>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstDealing_Vendors.add(Dealing_Vendors.fromJson(element.data() as Map<String, dynamic> )); });
	return lstDealing_Vendors;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Dealing_Vendors.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Dealing_Vendors.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Dealing_Vendors.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
