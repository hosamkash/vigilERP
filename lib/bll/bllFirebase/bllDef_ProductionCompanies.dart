import '../classModel/Def_ProductionCompanies.dart';
import 'dart:io';
...
class bllDef_ProductionCompanies
	{
		static List<String> ColumnsName = enTable_Def_ProductionCompanies.values.map((item) => item.name).toList();
			static late List<Def_ProductionCompanies> lstDef_ProductionCompanies = [];
			static late Def_ProductionCompanies itemDef_ProductionCompanies;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Def_ProductionCompanies.name)
	.orderBy(enTable_Def_ProductionCompanies.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Def_ProductionCompanies.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Def_ProductionCompanies colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Def_ProductionCompanies  itemDef_ProductionCompanies) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name).add(itemDef_ProductionCompanies.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemDef_ProductionCompanies) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name).doc(docName).set(itemDef_ProductionCompanies).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Def_ProductionCompanies  itemDef_ProductionCompanies) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name).doc(docName).set(itemDef_ProductionCompanies.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Def_ProductionCompanies  itemDef_ProductionCompanies,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemDef_ProductionCompanies.toMap());
...
static Future fire_setListMaster({
	required List<Def_ProductionCompanies> lstDef_ProductionCompanies,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstDef_ProductionCompanies.forEach((elm){
...
static Future<Def_ProductionCompanies> fire_getItem(String docName) async {
	late Def_ProductionCompanies  itemDef_ProductionCompanies;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name).doc(docName).get();
	 itemDef_ProductionCompanies = Def_ProductionCompanies();
 itemDef_ProductionCompanies.ID = retValue.data()!['ID']; itemDef_ProductionCompanies.Name = retValue.data()!['Name']; itemDef_ProductionCompanies.IsActive = retValue.data()!['IsActive']; itemDef_ProductionCompanies.IsSalesCategory = retValue.data()!['IsSalesCategory'];	return  itemDef_ProductionCompanies;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Def_ProductionCompanies>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Def_ProductionCompanies.name)
	.get()
	.then((value) {
	lstDef_ProductionCompanies.clear();
	value.docs.forEach((element) {
	lstDef_ProductionCompanies.add(Def_ProductionCompanies.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstDef_ProductionCompanies;
}

static Future<List<Def_ProductionCompanies>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstDef_ProductionCompanies.clear();value.docs.forEach((element) {
	lstDef_ProductionCompanies.add(Def_ProductionCompanies.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstDef_ProductionCompanies;
}

static Future<List<Def_ProductionCompanies>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstDef_ProductionCompanies.add(Def_ProductionCompanies.fromJson(element.data() as Map<String, dynamic> )); });
	return lstDef_ProductionCompanies;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_ProductionCompanies.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Def_ProductionCompanies.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Def_ProductionCompanies.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
