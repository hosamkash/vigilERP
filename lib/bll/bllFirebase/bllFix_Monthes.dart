import '../classModel/Fix_Monthes.dart';
import 'dart:io';
...
class bllFix_Monthes
	{
		static List<String> ColumnsName = enTable_Fix_Monthes.values.map((item) => item.name).toList();
			static late List<Fix_Monthes> lstFix_Monthes = [];
			static late Fix_Monthes itemFix_Monthes;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_Monthes.name)
	.orderBy(enTable_Fix_Monthes.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_Monthes.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_Monthes colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_Monthes  itemFix_Monthes) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name).add(itemFix_Monthes.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_Monthes) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name).doc(docName).set(itemFix_Monthes).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_Monthes  itemFix_Monthes) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name).doc(docName).set(itemFix_Monthes.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_Monthes  itemFix_Monthes,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_Monthes.toMap());
...
static Future fire_setListMaster({
	required List<Fix_Monthes> lstFix_Monthes,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_Monthes.forEach((elm){
...
static Future<Fix_Monthes> fire_getItem(String docName) async {
	late Fix_Monthes  itemFix_Monthes;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name).doc(docName).get();
	 itemFix_Monthes = Fix_Monthes();
 itemFix_Monthes.ID = retValue.data()!['ID']; itemFix_Monthes.Name = retValue.data()!['Name'];	return  itemFix_Monthes;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_Monthes>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_Monthes.name)
	.get()
	.then((value) {
	lstFix_Monthes.clear();
	value.docs.forEach((element) {
	lstFix_Monthes.add(Fix_Monthes.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_Monthes;
}

static Future<List<Fix_Monthes>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_Monthes.clear();value.docs.forEach((element) {
	lstFix_Monthes.add(Fix_Monthes.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_Monthes;
}

static Future<List<Fix_Monthes>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_Monthes.add(Fix_Monthes.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_Monthes;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_Monthes.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_Monthes.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_Monthes.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
