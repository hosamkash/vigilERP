import '../classModel/Def_Stocks.dart';
import 'dart:io';
...
class bllDef_Stocks
	{
		static List<String> ColumnsName = enTable_Def_Stocks.values.map((item) => item.name).toList();
			static late List<Def_Stocks> lstDef_Stocks = [];
			static late Def_Stocks itemDef_Stocks;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Def_Stocks.name)
	.orderBy(enTable_Def_Stocks.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Def_Stocks.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Def_Stocks colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Def_Stocks  itemDef_Stocks) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name).add(itemDef_Stocks.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemDef_Stocks) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name).doc(docName).set(itemDef_Stocks).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Def_Stocks  itemDef_Stocks) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name).doc(docName).set(itemDef_Stocks.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Def_Stocks  itemDef_Stocks,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemDef_Stocks.toMap());
...
static Future fire_setListMaster({
	required List<Def_Stocks> lstDef_Stocks,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstDef_Stocks.forEach((elm){
...
static Future<Def_Stocks> fire_getItem(String docName) async {
	late Def_Stocks  itemDef_Stocks;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name).doc(docName).get();
	 itemDef_Stocks = Def_Stocks();
 itemDef_Stocks.ID = retValue.data()!['ID']; itemDef_Stocks.Code = retValue.data()!['Code']; itemDef_Stocks.Name = retValue.data()!['Name']; itemDef_Stocks.IsActive = retValue.data()!['IsActive']; itemDef_Stocks.IsBindBranch = retValue.data()!['IsBindBranch']; itemDef_Stocks.IDBranch = retValue.data()!['IDBranch'];	return  itemDef_Stocks;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Def_Stocks>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Def_Stocks.name)
	.get()
	.then((value) {
	...
	lstDef_Stocks.add(Def_Stocks.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstDef_Stocks;
}

static Future<List<Def_Stocks>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstDef_Stocks.clear();value.docs.forEach((element) {
	lstDef_Stocks.add(Def_Stocks.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstDef_Stocks;
}

static Future<List<Def_Stocks>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstDef_Stocks.add(Def_Stocks.fromJson(element.data() as Map<String, dynamic> )); });
	return lstDef_Stocks;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Def_Stocks.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Def_Stocks.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Def_Stocks.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
