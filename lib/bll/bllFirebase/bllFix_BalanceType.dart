import '../classModel/Fix_BalanceType.dart';
import 'dart:io';
...
class bllFix_BalanceType
	{
		static List<String> ColumnsName = enTable_Fix_BalanceType.values.map((item) => item.name).toList();
			static late List<Fix_BalanceType> lstFix_BalanceType = [];
			static late Fix_BalanceType itemFix_BalanceType;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_BalanceType.name)
	.orderBy(enTable_Fix_BalanceType.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_BalanceType.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_BalanceType colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_BalanceType  itemFix_BalanceType) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name).add(itemFix_BalanceType.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_BalanceType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name).doc(docName).set(itemFix_BalanceType).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_BalanceType  itemFix_BalanceType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name).doc(docName).set(itemFix_BalanceType.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_BalanceType  itemFix_BalanceType,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_BalanceType.toMap());
...
static Future fire_setListMaster({
	required List<Fix_BalanceType> lstFix_BalanceType,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_BalanceType.forEach((elm){
...
static Future<Fix_BalanceType> fire_getItem(String docName) async {
	late Fix_BalanceType  itemFix_BalanceType;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name).doc(docName).get();
	 itemFix_BalanceType = Fix_BalanceType();
 itemFix_BalanceType.ID = retValue.data()!['ID']; itemFix_BalanceType.Name = retValue.data()!['Name'];	return  itemFix_BalanceType;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_BalanceType>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_BalanceType.name)
	.get()
	.then((value) {
	lstFix_BalanceType.clear();
	value.docs.forEach((element) {
	lstFix_BalanceType.add(Fix_BalanceType.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_BalanceType;
}

static Future<List<Fix_BalanceType>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_BalanceType.clear();value.docs.forEach((element) {
	lstFix_BalanceType.add(Fix_BalanceType.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_BalanceType;
}

static Future<List<Fix_BalanceType>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_BalanceType.add(Fix_BalanceType.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_BalanceType;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BalanceType.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_BalanceType.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_BalanceType.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
