import '../classModel/Fix_MaritalStatus.dart';
import 'dart:io';
...
class bllFix_MaritalStatus
	{
		static List<String> ColumnsName = enTable_Fix_MaritalStatus.values.map((item) => item.name).toList();
			static late List<Fix_MaritalStatus> lstFix_MaritalStatus = [];
			static late Fix_MaritalStatus itemFix_MaritalStatus;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_MaritalStatus.name)
	.orderBy(enTable_Fix_MaritalStatus.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_MaritalStatus.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_MaritalStatus colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_MaritalStatus  itemFix_MaritalStatus) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name).add(itemFix_MaritalStatus.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_MaritalStatus) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name).doc(docName).set(itemFix_MaritalStatus).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_MaritalStatus  itemFix_MaritalStatus) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name).doc(docName).set(itemFix_MaritalStatus.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_MaritalStatus  itemFix_MaritalStatus,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_MaritalStatus.toMap());
...
static Future fire_setListMaster({
	required List<Fix_MaritalStatus> lstFix_MaritalStatus,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_MaritalStatus.forEach((elm){
...
static Future<Fix_MaritalStatus> fire_getItem(String docName) async {
	late Fix_MaritalStatus  itemFix_MaritalStatus;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name).doc(docName).get();
	 itemFix_MaritalStatus = Fix_MaritalStatus();
 itemFix_MaritalStatus.ID = retValue.data()!['ID']; itemFix_MaritalStatus.Name = retValue.data()!['Name'];	return  itemFix_MaritalStatus;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_MaritalStatus>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_MaritalStatus.name)
	.get()
	.then((value) {
	lstFix_MaritalStatus.clear();
	value.docs.forEach((element) {
	lstFix_MaritalStatus.add(Fix_MaritalStatus.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_MaritalStatus;
}

static Future<List<Fix_MaritalStatus>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_MaritalStatus.clear();value.docs.forEach((element) {
	lstFix_MaritalStatus.add(Fix_MaritalStatus.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_MaritalStatus;
}

static Future<List<Fix_MaritalStatus>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_MaritalStatus.add(Fix_MaritalStatus.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_MaritalStatus;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_MaritalStatus.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_MaritalStatus.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_MaritalStatus.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
