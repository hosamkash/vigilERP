import '../classModel/Fix_BounsType.dart';
import 'dart:io';
...
class bllFix_BounsType
	{
		static List<String> ColumnsName = enTable_Fix_BounsType.values.map((item) => item.name).toList();
			static late List<Fix_BounsType> lstFix_BounsType = [];
			static late Fix_BounsType itemFix_BounsType;
			static String query = '';
...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_BounsType.name)
	.orderBy(enTable_Fix_BounsType.ID.name, descending: true)
	.limit(1).get().then((value) {
	if (value.docs.isEmpty) {maxID = 1;}
	else {
	maxID = value.docs.first.get(enTable_Fix_BounsType.ID.name);
	maxID += 1;}
	}).catchError((error) {
...
static Future<int> getMax_firestore(enTable_Fix_BounsType colName, {BLLCondions? condion}) async {
	int maxID = 1;
...
static Future<String> fire_AddItem(Fix_BounsType  itemFix_BounsType) async {
	String docCreatedName = '';
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name).add(itemFix_BounsType.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_BounsType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name).doc(docName).set(itemFix_BounsType).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_BounsType  itemFix_BounsType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name).doc(docName).set(itemFix_BounsType.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	required Fix_BounsType  itemFix_BounsType,
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name);
	DocumentReference tablemasterDocref = coll.doc(insertdDocID);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	// save Master
	batch.set(tablemasterDocref,  itemFix_BounsType.toMap());
...
static Future fire_setListMaster({
	required List<Fix_BounsType> lstFix_BounsType,
	}) async {
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name);
	WriteBatch batch = FirebaseFirestore.instance.batch();
	lstFix_BounsType.forEach((elm){
...
static Future<Fix_BounsType> fire_getItem(String docName) async {
	late Fix_BounsType  itemFix_BounsType;
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name).doc(docName).get();
	 itemFix_BounsType = Fix_BounsType();
 itemFix_BounsType.ID = retValue.data()!['ID']; itemFix_BounsType.Name = retValue.data()!['Name'];	return  itemFix_BounsType;
	} catch (error) {
	print(error.toString()); throw error;
	}
}

static Future<List<Fix_BounsType>> fire_getList() async {
	await FirebaseFirestore.instance
	.collection(en_TablesName.Fix_BounsType.name)
	.get()
	.then((value) {
	lstFix_BounsType.clear();
	value.docs.forEach((element) {
	lstFix_BounsType.add(Fix_BounsType.fromJson(element.data()));
	});
	}).catchError((error) {
print(error.toString()); throw error;
});
return lstFix_BounsType;
}

static Future<List<Fix_BounsType>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name)
	.where(BranchColumnName, isEqualTo: branchID).get()
	.then((value) {lstFix_BounsType.clear();value.docs.forEach((element) {
	lstFix_BounsType.add(Fix_BounsType.fromJson(element.data()));});
	}).catchError((error) {print(error.toString());throw error;});
	return lstFix_BounsType;
}

static Future<List<Fix_BounsType>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name);
	Query query = colRef;
	  if (conditions != null) {
	...
	lstFix_BounsType.add(Fix_BounsType.fromJson(element.data() as Map<String, dynamic> )); });
	return lstFix_BounsType;
}
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_BounsType.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_BounsType.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_BounsType.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
