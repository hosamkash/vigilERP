...
static Future<int> getMaxID_firestore() async {
	...
	.collection(en_TablesName.Fix_FinacialType.name)
...
static Future<String> fire_AddItem(Fix_FinacialType  itemFix_FinacialType) async {
	...
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_FinacialType.name).add(itemFix_FinacialType.toMap()).then((val) {
...
static Future<bool> fire_SetItemMap(String docName, Map<String, dynamic>  itemFix_FinacialType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_FinacialType.name).doc(docName).set(itemFix_FinacialType).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future<bool> fire_SetItem(String docName, Fix_FinacialType  itemFix_FinacialType) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_FinacialType.name).doc(docName).set(itemFix_FinacialType.toMap()).then((val) {
	result = true;
	}).catchError((error) {
	print(error.toString()); throw error;
	});
...
static Future fire_setListMaster_And_Details(
	...
	CollectionReference coll = FirebaseFirestore.instance.collection(en_TablesName.Fix_FinacialType.name);
...
static Future<Fix_FinacialType> fire_getItem(String docName) async {
	...
	try { var retValue = await FirebaseFirestore.instance.collection(en_TablesName.Fix_FinacialType.name).doc(docName).get();
...
static Future<List<Fix_FinacialType>> fire_getListByBranch(int branchID , String BranchColumnName) async {
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_FinacialType.name)
...
static Future<List<Fix_FinacialType>> fire_getListWithConditions({List<BLLCondions>? conditions}) async {
	CollectionReference colRef = FirebaseFirestore.instance.collection(en_TablesName.Fix_FinacialType.name);
	Query query = colRef;
	  if (conditions != null) {
...
static Future<bool> fire_DeleteItem(String docName) async {
	bool result = false;
	await FirebaseFirestore.instance.collection(en_TablesName.Fix_FinacialType.name).doc(docName).delete().then((val) {
	result = true;
	}).catchError((error) {
...
static Future<String> storage_UploadFile(String fileName, File file ,{String pathOnStorage = ''}) async {
	pathOnStorage = pathOnStorage.isEmpty ? en_TablesName.Fix_FinacialType.name : pathOnStorage;
...
static Future<String> storage_GetFileDowenloadURL(String fileName) async {
	String fileDownloadURL = '';
	await firebase_storage.FirebaseStorage.instance.ref().child('${en_TablesName.Fix_FinacialType.name}/${fileName}').getDownloadURL().then((URL) {
	fileDownloadURL = URL;
	}).catchError((error) {
	print(error.toString());  throw error;
	});
